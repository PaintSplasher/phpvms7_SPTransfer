...
class HubFrontendController extends Controller
{
    ...
    public function index()
    {
        ...
        $last_transfer = DB_SPTransfer::where('user_id', $user->id)->where('airline', 0)->latest()->first();
    ...
    public function store(Request $request)
    {
        ...
        if ($user->home_airport_id === $request->hub_request_id) {
            ...
            return redirect(route('sptransfer.hub.index'));
        }
        ...
        Log::debug('SPTransfer(HUB) | Transfer from '.strtoupper($sptransfer->hub_initial_id).' to '.strtoupper($sptransfer->hub_request_id).' requested by '.$user->name_private);

        event(new HubChangeRequest($sptransfer));

        flash()->success('HUB Transfer request submitted.');
    ...
    public function ChargeForFreeFlight($user, $amount, $memo)
    {
        ...
        // Note Transaction
        Log::debug('SPTransfer(HUB) | UserID: '.$user->id.' Name: '.$user->name_private.' charged for '.$memo.' on request.');
    }
}
